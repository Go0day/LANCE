...
def replace_rewrite_news(tmp_news_dataset, rewrited_news_collection, position):
    for ori_idx in tmp_news_dataset.news2dict:
        for re_news_idx, re_news_id in enumerate(rewrited_news_collection['news_ID']):
            if tmp_news_dataset.news2dict[ori_idx]['id'] == re_news_id:
                tokenized_title = tokenizer(rewrited_news_collection['news_title'][re_news_idx][position].lower(), max_length=config.num_words_title,padding='max_length', truncation=True)
                ...
                if "abstract" in tmp_news_dataset.news2dict[0].keys():
                    tokenized_abstract = tokenizer(rewrited_news_collection['news_abstract'][re_news_idx][position].lower(), max_length=config.num_words_abstract,padding='max_length', truncation=True)
...
def compute_rank_metrics(target_original_path, target_rewritten_path, test_directory, model):
    ...
    result = {
        ...
        "rewrited": {
            "index": rewrited_news_collection["index"],
            "news_ID": rewrited_news_collection["news_ID"],
    ...
    cur_win_lose_index = rewrited_news_collection['index']
    top_medium_bottom = [list(set(target_test['top_index']).intersection(cur_win_lose_index)), list(set(target_test['medium_index']).intersection(cur_win_lose_index)), list(set(target_test['bottom_index']).intersection(cur_win_lose_index))]
    
    print(" ===== Win or Lose Information is ===== ")
    print("Total News: ", len(cur_win_lose_index), "Top News: ", len(top_medium_bottom[0]), "Medium News: ", len(top_medium_bottom[1]), "Bottom News: ", len(top_medium_bottom[2]))
    print(" ===== Win or Lose Information Done ===== ")
    
    K_values = [10, 20, 50, 100]
    result["original"]["result"] = calculate_metrics_on_original(
            result["rewrited"]["news_ID"], original_user2vector, original_news2vector, batch_size=512, K=K_values
        )
    ...
    print(f"Total {num_rewrite} rewritten versions.")
    ...
    for i in range(num_rewrite):
        ...
        result["rewrited"]["result"][i] = calculate_metrics(
            result["rewrited"]["news_ID"], original_user2vector, original_news2vector, tmp_rewrited_user2vector, tmp_rewrited_news2vector, batch_size=512, K=K_values
        )
...
if __name__ == '__main__':
    ...
    # rewrited_target_path = path.join(target_dir, f'rewrited_target_' + config.target_trainORtest + "_" + config.diverse_mode + "_" + config.llm_api_mode + '.pkl')

    rewrited_target_path_train_win = path.join(target_dir, model_name + 'train_win_rewrited_target_' + config.target_trainORtest + "_" + config.diverse_mode + "_" + config.llm_api_mode + '.pkl')
    rewrited_target_path_train_reject = path.join(target_dir, model_name + 'train_reject_rewrited_target_' + config.target_trainORtest + "_" + config.diverse_mode + "_" + config.llm_api_mode + '.pkl')

    print (" ======================================== WIN ======================================== ")
    result_attack = compute_rank_metrics(
        target_original_path=target_path,
        target_rewritten_path=rewrited_target_path_train_win,
    ...
    result_attack = compute_rank_metrics(
        target_original_path=target_path,
        target_rewritten_path=rewrited_target_path_train_reject,
        test_directory=test_directory,
        model=model
    )
