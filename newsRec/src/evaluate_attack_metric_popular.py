# This is a function to get the attack performance among four metrics, GPT
#       {
    #     "Exposure": exposure,
...
if __name__ == '__main__':
    ...
    if config.llm_test_version != 0:
        target_gpt_dir = path.join(target_dir, config.llm_name + "V" + str(config.llm_test_version))
    else:
        target_gpt_dir = path.join(target_dir, config.llm_name)
    target_path = path.join(target_dir, train_or_test[config.target_trainORtest])
    tmp_model_name = model_name
    model_name = 'ALL'
    rewrited_target_path = path.join(target_gpt_dir, 'rewrited_target_popular_ALL_' + config.news_encoder + "_" + config.target_trainORtest + "_" + config.diverse_mode + '.pkl')
